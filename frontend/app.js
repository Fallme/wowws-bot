...
async function pollStatus(){
  try{
    ...
    const intervention=data.state==="failed";$("#interventionCard").hidden=!intervention;$("#interventionMessage").textContent=intervention?`${data.message||"自动流程失败"}${data.error?`：${data.error}`:""}`:"";const logOutput=$("#logOutput");logOutput.textContent=(data.log||[]).join("\n")||"[SYSTEM] 暂无运行日志";requestAnimationFrame(()=>{logOutput.scrollTop=logOutput.scrollHeight;});
